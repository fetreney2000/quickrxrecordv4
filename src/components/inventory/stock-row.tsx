 ...
 *
 * Desktop: grid 5 lajur (Kod, Nama + Kekuatan + Bentuk + Dagangan, Kuota, Stok, Tindakan)
 * Mudah alih: card dengan kod mono, nama, dan lencana stok
 */
...
export function StockRow({ item, index, onClick }: StockRowProps) {
  ...
  return (
    <>
        ...
        <div>
          ...
          >
            <span
              ...
              style={{ background: stock > 0 ? "#16a34a" : "#e41e3f" }}
            />
            {formatNumber(stock)}
          </span>
        </div>
      ...
      >
        <div
          className="w-10 h-10 rounded-[10px] flex items-center justify-center text-white text-xs font-bold flex-shrink-0"
          style={{
            background: "linear-gradient(135deg, #7c3aed, #6d28d9)",
          }}
        >
          <span className="font-mono" style={{ fontSize: 10 }}>
            {item.kod_item.slice(0, 3).toUpperCase()}
          </span>
        </div>
