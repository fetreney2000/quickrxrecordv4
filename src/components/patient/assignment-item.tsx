...
function SortableHeader({ label, sortKey, currentSort, onSort }: { label: string; sortKey: string; currentSort: { key: string; dir: SortDir } | null; onSort: (key: string) => void; }) {
  ...
  return (
    <th onClick={() => onSort(sortKey)} className="text-left text-2xs font-semibold uppercase tracking-wider px-2 py-1.5 cursor-pointer select-none hover:bg-muted/50 transition-colors" style={{ color: "#65676b" }}>
  ...
  return (
    <div className="py-3 px-2">
      <div className="flex items-start gap-3">
        <div className="w-9 h-9 rounded-xl flex items-center justify-center text-white flex-shrink-0" style={{ background: assignment.aktif ? "linear-gradient(135deg, #1877f2, #0d5bd4)" : "#9ca3af", opacity: assignment.aktif ? 1 : 0.5 }}>
          <Pill className="w-4 h-4" />
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2 flex-wrap">
            <div className="min-w-0">
              <p className="text-sm font-bold truncate" style={{ color: "#1c1e21" }}>
                {item?.nama_item ?? "Item Tidak Dikenali"}
                {item?.kekuatan ? <span className="text-xs font-medium ml-1" style={{ color: "#65676b" }}>· {item.kekuatan}</span> : null}
                {formName ? <span className="text-xs font-medium ml-1" style={{ color: "#65676b" }}>· {formName}</span> : null}
              </p>
              <div className="flex items-center gap-2 mt-0.5 text-xs flex-wrap" style={{ color: "#65676b" }}>
                ...
                {assignment.dos && <span>· Dos: <strong style={{ color: "#1c1e21" }}>{assignment.dos}</strong></span>}
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-1 mt-2 text-2xs" style={{ color: "#65676b" }}>
                ...
                {assignment.tarikh_tamat_guna && <span>Tamat: <strong style={{ color: "#1c1e21" }}>{formatDate(assignment.tarikh_tamat_guna)}</strong></span>}
                {assignment.dimulakan_oleh_profile?.nama && <span>Dimulai: <strong style={{ color: "#1c1e21" }}>{assignment.dimulakan_oleh_profile.nama}</strong></span>}
                {assignment.ditamatkan_oleh_profile?.nama && <span>Ditamatkan: <strong style={{ color: "#1c1e21" }}>{assignment.ditamatkan_oleh_profile.nama}</strong></span>}
              </div>
          ...
          {assignment.catatan_penggunaan && <p className="text-xs mt-1 italic" style={{ color: "#65676b" }}>{assignment.catatan_penggunaan}</p>}
          {assignment.sebab_tamat && <p className="text-xs mt-1 italic" style={{ color: "#9ca3af" }}>Sebab tamat: {assignment.sebab_tamat}</p>}
                  ...
                  <tbody>
                    {sortedDose.map((d) => (
                      <tr key={d.id} className="border-b last:border-b-0" style={{ borderColor: "#f0f2f5" }}>
                        ...
                        <td className="px-2 py-1.5" style={{ color: "#65676b" }}>{(d as DoseHistoryWithProfile).dikemaskini_oleh_profile?.nama ?? "—"}</td>
                        <td className="px-2 py-1.5 italic" style={{ color: "#9ca3af" }}>{d.catatan || "—"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                  ...
                  <tbody>
                    {sortedSupply.map((s) => (
                      <tr key={s.id} className="border-b last:border-b-0" style={{ borderColor: "#f0f2f5" }}>
                        ...
                        <td className="px-2 py-1.5" style={{ color: "#65676b" }}>{(s as SupplyRecordWithJoins).kakitangan_pembekal_profile?.nama ?? "—"}</td>
                        <td className="px-2 py-1.5 italic" style={{ color: "#9ca3af" }}>{s.catatan_bekalan || "—"}</td>
