...
export default function StockListPage() {
  ...
  const handleItemClick = useCallback((item: Item) => { const displayTitle = [item.nama_item, item.kekuatan].filter(Boolean).join(" "); setBreadcrumbTrail([HOME_CRUMB, { label: "Senarai Inventori", href: "/stok" }, { label: displayTitle }]); navigate(`/stok/${item.id}`); }, [navigate, setBreadcrumbTrail]);
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0"><h1 className="text-[22px] font-bold leading-tight truncate" style={{ color: "#1c1e21", letterSpacing: "-0.01em" }}>Senarai Inventori</h1><p className="text-[13px] font-medium mt-0.5" style={{ color: "#65676b" }}>Urus item ubat dalam katalog</p></div>
        </div>
      ...
      <div><Card><CardContent className="p-0 relative">
        <div className="p-4 sm:p-5 pb-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-[#f0f2f5]">
          <div className="flex items-center gap-2 flex-1 min-w-0">
            <div className="relative flex-1 min-w-0" style={{ maxWidth: 400 }}>
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4" style={{ color: searchFocused ? "#7c3aed" : "#9ca3af" }} />
            ...
            <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-xl text-[12px] font-semibold flex-shrink-0" style={{ background: "rgba(124,58,237,0.06)", color: "#65676b", border: "1px solid rgba(124,58,237,0.10)" }}><span style={{ color: "#7c3aed" }}>{total.toLocaleString("ms-MY")}</span><span>item</span></div>
            {isFetching && !isLoading && <Loader2 className="w-3.5 h-3.5 animate-spin" style={{ color: "#7c3aed" }} />}
          </div>
        </div>
        <div className="relative">
          {isLoading && <div className="flex flex-col items-center justify-center py-12 gap-2" style={{ color: "#65676b" }}><Loader2 className="w-6 h-6 animate-spin" style={{ color: "#7c3aed" }} /><p className="text-sm">Memuatkan item...</p></div>}
          {!isLoading && items.length === 0 && debouncedSearch && <div className="flex flex-col items-center justify-center py-12 gap-2" style={{ color: "#9ca3af" }}><Inbox className="w-10 h-10 opacity-40" /><p className="text-sm font-medium" style={{ color: "#65676b" }}>Tiada item dijumpai.</p><p className="text-xs">Cuba tukar kata kunci carian anda.</p></div>}
          {!isLoading && items.length === 0 && !debouncedSearch && <div className="flex flex-col items-center justify-center py-12 gap-2" style={{ color: "#9ca3af" }}><Pill className="w-10 h-10 opacity-40" /><p className="text-sm font-medium" style={{ color: "#65676b" }}>Tiada item dalam inventori.</p>{canEdit && <p className="text-xs">Klik "Tambah Item" untuk mendaftarkan item baru.</p>}</div>}
          {!isLoading && items.length > 0 && (
            <>
              <div className="hidden sm:grid stok-table-header px-4 py-2.5 text-2xs font-semibold uppercase tracking-wider" style={{ gridTemplateColumns: "1.2fr 3fr 1fr 1fr 1fr", gap: 12, color: "#65676b", background: "rgba(0,0,0,0.02)", borderBottom: "2px solid #e4e6eb" }}>
                ...
                <div className="flex items-center gap-1.5" style={{ color: "#65676b" }}><Package className="w-3 h-3" /><span>Stok</span></div>
                <div>Baki Kuota</div>
              </div>
        ...
        {!isLoading && totalPages > 1 && (
          <div className="px-4 py-3 border-t border-[#f0f2f5] flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <p className="text-xs" style={{ color: "#65676b" }}>Halaman {page + 1} daripada {totalPages} ({total.toLocaleString("ms-MY")} item)</p>
        ...
        {!isLoading && totalPages === 1 && total > 0 && <div className="px-4 py-2 border-t border-[#f0f2f5] text-xs" style={{ color: "#65676b" }}>{total.toLocaleString("ms-MY")} item · Halaman 1 daripada 1</div>}
...
function SortableHeader({ columnKey, label, sort, onSort, icon: Icon }: SortableHeaderProps) {
  ...
  return <button type="button" onClick={() => onSort(columnKey)} className="flex items-center gap-1.5 hover:text-foreground transition-colors text-left" style={{ color: isActive ? "#7c3aed" : "#65676b" }}><Icon className="w-3 h-3" /><span>{label}</span><SortIcon active={isActive} dir={sort?.dir ?? "asc"} /></button>;
}